...
This runs `rsync` to copy tool assets from `tool/` into `public/tool/`, then
...
Create a new directory `tool/TOOLNAME/` with an `index.yaml` file. The directory
...
Put screenshot images in `tool/TOOLNAME/` alongside `index.yaml`.
